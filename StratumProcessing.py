...
from Config import *
from StratumProtocol import StratumProtocol
...
class StratumProcessing:
    ...
    def tx_make_coinbase(self, coinbase_script, extranonce):
        ...
        tx = "01000000" + "01" + "0" * 64 + "ffffffff" + self.int2varinthex(
            (len(coinbase_script) + len(extranonce)) // 2) + coinbase_script + extranonce + "ffffffff" + "01" + self.int2lehex(self.fee, 8) + \
             self.int2varinthex(len(pubkey_script) // 2) + pubkey_script + "00000000"
        return tx

    def tx_make_coinbase_stratum(self, coinbase_script, extranonce_placeholder):
        pubkey_script = "76" + "a9" + "14" + self.bitcoinaddress2hash160(get_wallet_address()) + "88" + "ac"
        coinbase_script = self.tx_encode_coinbase_height(self.height) + coinbase_script

        coinbase1 = "01000000" + "01" + "0" * 64 + "ffffffff" + self.int2varinthex(
            (len(coinbase_script) + len(extranonce_placeholder)) // 2) + coinbase_script + extranonce_placeholder # whitout extranonce

        coinbase2 = "ffffffff" + "01" + self.int2lehex(self.fee, 8) + \
                    self.int2varinthex(len(pubkey_script) // 2) + pubkey_script + "00000000"

        return coinbase1, coinbase2

    def tx_make_coinbase_stratum_raw(self, coinbase_script):
        pubkey_script = "76" + "a9" + "14" + self.bitcoinaddress2hash160(get_wallet_address()) + "88" + "ac"
        coinbase_script = self.tx_encode_coinbase_height(self.height) + coinbase_script

        coinbase1 = "01000000" + "01" + "0" * 64 + "ffffffff" + self.int2varinthex(
            len(coinbase_script) // 2) + coinbase_script
        # whitout extranonce
        coinbase2 = "ffffffff" + "01" + self.int2lehex(self.fee, 8) + \
                    self.int2varinthex(len(pubkey_script) // 2) + pubkey_script + "00000000"
    ...
    # def block_make_header_stratum(self, merkleroot, nonce, ntime):
    #     header = b""
    #     header += struct.pack("<L", int(self.version, 16))
    #     header += bytes.fromhex(self.prevhash) #[::-1]
    #     header += bytes.fromhex(merkleroot) #[::-1]
    #     header += struct.pack("<L", ntime)
    #     header += bytes.fromhex(self.nbits) #[::-1]
    #     header += struct.pack("<L", nonce)
    #     return header

    def block_make_header(self, merkleroot, nonce, ntime):
        header = b""
    ...
    def block_make_submit(self, transactions, merkleroot, nonce, ntime):
        """
        ...
        # Block header
        submission += self.block_make_header(merkleroot, nonce, ntime).hex()
    ...
    async def block_validate(self, coinbase_message, transactions, merkleroot, extranonce2, ntime, nonce):

        if type(extranonce2) != int:
            extranonce2 = int(extranonce2, 16)

        # Coinbase sin stratum
        # coinbase_script = self.to_coinbase_script(coinbase_message)  # extranonce: 4554
        # coinbase1 = self.tx_make_coinbase(coinbase_script, extranonce2.to_bytes(4, byteorder='big').hex())
        # coinbase_script2 = hashlib.sha256(hashlib.sha256(bytes.fromhex(coinbase1)).digest()).digest().hex()
        # coinbase2_hash = self.to_little_endian(coinbase_script2)

        # Coinbase para stratum
        coinbase1, coinbase2 = self.tx_make_coinbase_stratum(coinbase_message, extranonce2.to_bytes(4, byteorder='big').hex())
        coinbase2_hash = hashlib.sha256(hashlib.sha256(bytes.fromhex(coinbase1 + coinbase2)).digest()).digest().hex()
        coinbase2_hash = self.to_little_endian(coinbase2_hash)
        ...
        # transactions1 = tx
        # merkle1.insert(0, coinbase2_hash)
        merkle1 = tx
        # merkle1[0] = coinbase2_hash

        merkleroot = self.tx_compute_merkle_root(merkle1)
        ...
        block_header = self.block_make_header(merkleroot, nonce, ntime)
        block_header = block_header[0:76] + int(nonce).to_bytes(4, byteorder='little')  # bytes.fromhex(nonce)
        block_hash = self.block_compute_raw_hash(block_header)
        print(block_hash.hex())

        if block_hash.hex().startswith(10 * "0") or block_hash.hex().endswith(10 * "0"):
            self.hash = block_hash.hex()
            print("Solved a block! Block hash: {}".format(self.hash))
            submission = self.block_make_submit(transactions, merkleroot, nonce, ntime)
            result = await BlockTemplateFetcher.submitblock(submission)
            # result = True
            return result
        return False
    ...
    async def create_job_machine(self, model, time_block, negative):
        # global tx
        # self.prevhash = "000000000000015f416afc2a44461adb178764a4fb45e5935c0a5717edf451a8"
        # self.version = "00000002" #int(2).to_bytes(4, byteorder='big').hex()[::-1]
        # self.nbits = "1a01aa3d"
        # self.fee = 2505860000
        # self.height = 235759
        # self.ntime = int(1368328721).to_bytes(4, byteorder='big').hex()
        # ntime = int(self.ntime, 16)
        # nonce = int("8A07BC5A", 16)
        # extranonce2 = int(4554).to_bytes(4, byteorder='big').hex()

        version = self.version.to_bytes(4, byteorder='big').hex()[::-1]
        ...
        extranonce2 = random.randint(1, 16 ** 8)

        # Coinbase para cpu/gpu
        # coinbase_script = self.to_coinbase_script(coinbase_message)  # extranonce: 4554
        # coinbase1 = self.tx_make_coinbase(coinbase_script, extranonce2.to_bytes(4, byteorder='big').hex())
        # coinbase_script2 = hashlib.sha256(hashlib.sha256(bytes.fromhex(coinbase1)).digest()).digest().hex()
        # coinbase2_hash = self.to_little_endian(coinbase_script2)
        ...
        if type(self.ntime) != int:
            ntime = int(self.ntime, 16).to_bytes(4, byteorder='big').hex()
        else:
            ntime = self.ntime
        ...
        stratum_protocol = StratumProtocol(self, coinbase_message, merkle1, merkleroot1, job_data)
        # asyncio.run(stratum_protocol.run_server(self, coinbase_message, merkle1, merkleroot1, job_data))
        await stratum_protocol.run_server(self, coinbase_message, merkle1, merkleroot1, job_data)

        # submission = self.block_validate(coinbase_message, transactions1, merkleroot1, extranonce2, ntime, nonce) #
